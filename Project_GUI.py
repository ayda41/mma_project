...
def create_transaction(project, amount, people, payer, method, description, category, *args):
    ...
    if method == 'equal':
        ...
        project_update_balance(project, balance)
...
def project_update_balance(project, balance):
    user_database = shelve.open('user_database')
    for friend, amount in balance.items():
        user = user_database[friend.name]
        project.balance[user] = project.balance[friend]
        project.balance[user] += amount
        del project.balance[friend]
...
def update_friend(new_user):
    ...
    for name, user in user_database.items():
        ...
        user_database[new_user.name].add_friends(user)
...
class Project():
    ...
    def return_transactions(self):
        return self.ledger[["date", "total_amount", "people_name", "payer_name", "method", "description","category", "Split"]].copy().drop([0])
